...
let anotherWidthLabel = "The width is \(width + 5)"
//: ## Collections
//: ### Arrays
...
moreCaptains.append("Katheryn Janeway")
print(moreCaptains)
moreCaptains[1] = "Poe Dameron"
// ^^ Replaces the value at that index.  Use insert to add, instead of replace.
print(moreCaptains)

moreCaptains.insert("Jean Luc Picard", at: 1)
//: ### Dictionaries
...
// Since dictionaries are unordered, when a change is added, the key/value pair does not overwrite the existing value at that index
...
// ^^^^ Keep in mind in this example, that the result is returned as an Interger, so the result might not be correct because the floating points are truncated.

func averageScoreHighPrecision(_ scores: [Int]) -> Double {
...
let avgHP = averageScoreHighPrecision(scores)
//: ## Tuples
...
// Get a total count of all the items in the store
